...
class MarginfiAccount {
    ...
    static fromAccountDataRaw(marginfiAccountPk, marginfiAccountRawData, idl) {
    ...
    computeHealthComponents(banks, oraclePrices, marginReqType, excludedBanks = []) {
        ...
        const [assets, liabilities] = filteredBalances
            ...
            if (!bank)
                throw Error(`1Bank ${(0, mrgn_common_1.shortenAddress)(accountBalance.bankPk)} not found`);
            const priceInfo = oraclePrices.get(accountBalance.bankPk.toBase58());
            if (!priceInfo)
                throw Error(`2Bank ${(0, mrgn_common_1.shortenAddress)(accountBalance.bankPk)} not found`);
    ...
    getHealthCheckAccounts(banks, mandatoryBanks = [], excludedBanks = [], bankMetadataMap) {
    ...
    // Actions
    // ----------------------------------------------------------------------------
    async makeDepositIx(program, banks, mintDatas, amount, bankAddress, opt = {}) {
        const bank = banks.get(bankAddress.toBase58());
        if (!bank)
            throw Error(`Bank ${bankAddress.toBase58()} not found`);
        ...
        const depositIxs = bank.mint.equals(mrgn_common_1.NATIVE_MINT) && wrapAndUnwrapSol ? this.wrapInstructionForWSol(ix, amount) : [ix];
        return {
            instructions: depositIxs,
            keys: [],
        };
    }
    async makeRepayIx(program, banks, mintDatas, amount, bankAddress, repayAll = false, opt = {}) {
        const bank = banks.get(bankAddress.toBase58());
        if (!bank)
            throw Error(`Bank ${bankAddress.toBase58()} not found`);
        const mintData = mintDatas.get(bankAddress.toBase58());
        ...
        if (repayAll && !bank.emissionsMint.equals(web3_js_1.PublicKey.default)) {
            if (createAtas) {
                const userAta = (0, mrgn_common_1.getAssociatedTokenAddressSync)(bank.emissionsMint, this.authority, true, mintData.tokenProgram); // We allow off curve addresses here to support Fuse.
                const createAtaIdempotentIx = (0, mrgn_common_1.createAssociatedTokenAccountIdempotentInstruction)(this.authority, userAta, this.authority, bank.emissionsMint, mintData.tokenProgram);
    ...
    async makeWithdrawIx(program, banks, mintDatas, bankMetadataMap, amount, bankAddress, withdrawAll = false, opt = {}) {
        const bank = banks.get(bankAddress.toBase58());
        if (!bank)
            throw Error(`Bank ${bankAddress.toBase58()} not found`);
        const mintData = mintDatas.get(bankAddress.toBase58());
        ...
        const userAta = (0, mrgn_common_1.getAssociatedTokenAddressSync)(bank.mint, this.authority, true, mintData.tokenProgram);
        if (createAtas) {
        ...
        if (opt.observationBanksOverride !== undefined) {
            remainingAccounts.push(...makeHealthAccountMetas(banks, opt.observationBanksOverride, bankMetadataMap, this.authority));
        }
        else {
    ...
    async makeBorrowIx(program, banks, mintDatas, bankMetadataMap, amount, bankAddress, opt = {}) {
        const bank = banks.get(bankAddress.toBase58());
        if (!bank)
            throw Error(`Bank ${bankAddress.toBase58()} not found`);
        const mintData = mintDatas.get(bankAddress.toBase58());
        ...
        const borrowIxs = bank.mint.equals(mrgn_common_1.NATIVE_MINT) && wrapAndUnwrapSol ? this.wrapInstructionForWSol(ix) : [ix];
        ixs.push(...borrowIxs);
        return {
            instructions: ixs,
            keys: [],
    ...
    async makeLendingAccountLiquidateIx(liquidateeMarginfiAccount, program, banks, mintDatas, bankMetadataMap, assetBankAddress, assetQuantityUi, liabilityBankAddress) {
        const assetBank = banks.get(assetBankAddress.toBase58());
        if (!assetBank)
        ...
        }, { assetAmount: (0, mrgn_common_1.uiToNative)(assetQuantityUi, assetBank.mintDecimals) }, remainingAccounts);
    ...
    async makeCloseAccountIx(program) {
        const ix = await instructions_1.default.makeCloseAccountIx(program, {
            marginfiAccountPk: this.address,
            feePayerPk: this.authority,
            authorityPk: this.authority,
        });
    ...
    describe(banks, oraclePrices) {
        ...
        const { assets: assetsInit, liabilities: liabilitiesInit } = this.computeHealthComponents(banks, oraclePrices, MarginRequirementType.Initial);
        let description = `
...
function makeHealthAccountMetas(banks, banksToInclude, bankMetadataMap, authority) {
    return banksToInclude.flatMap((bankAddress) => {
        ...
        if (bank.config.assetTag === 2) {
            if (!authority)
                throw Error("Authority is required for staked accounts");
            const solBank = Array.from(banks.values()).find((b) => b.mint.equals(mrgn_common_1.WSOL_MINT));
            if (!solBank)
                throw Error(`SOL bank not found`);
            ...
            const pool = (0, single_spl_pool_1.findPoolAddress)(new web3_js_1.PublicKey(bankMetadata.validatorVoteAccount));
            const solPool = (0, single_spl_pool_1.findPoolStakeAddress)(pool);
            const lstMint = (0, single_spl_pool_1.findPoolMintAddress)(pool);
            accs.push({ pubkey: lstMint, isSigner: false, isWritable: false }, { pubkey: solPool, isSigner: false, isWritable: false });
        }
        return accs;
    });
}
