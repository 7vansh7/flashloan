...
class MarginfiAccountWrapper {
     ...
     */
    async movePosition(amount, bankAddress, destinationAccount, processOpts, txOpts) {
     ...
     */
    async makeMovePositionTx(amount, bankAddress, destinationAccount) {
        ...
        const depositTx = (0, mrgn_common_1.addTransactionMetadata)(tx, {
            ...
            addressLookupTables: lookupTables,
            type: mrgn_common_1.TransactionType.MOVE_POSITION,
        });
     ...
     */
    async makeRepayWithCollatTxV2({ repayAmount, withdrawAmount, borrowBankAddress, depositBankAddress, withdrawAll = false, repayAll = false, swap, blockhash: blockhashArg, withdrawOpts, repayOpts, }) {
        ...
        const withdrawIxs = await this.makeWithdrawIx(withdrawAmount, depositBankAddress, withdrawAll, withdrawOpts ?? {
            createAtas: false,
            wrapAndUnwrapSol: false,
        });
        const repayIxs = await this.makeRepayIx(repayAmount, borrowBankAddress, repayAll, repayOpts ?? {
            createAtas: false,
            wrapAndUnwrapSol: false,
        });
        const { instructions: updateFeedIxs, luts: feedLuts } = await this.makeUpdateFeedIx([
    ...
    async makeLoopTxV2({ depositAmount, borrowAmount, depositBankAddress, borrowBankAddress, swap, blockhash: blockhashArg, depositOpts, borrowOpts, setupBankAddresses, }) {
        const depositBank = this.client.banks.get(depositBankAddress.toBase58());
        if (!depositBank)
            throw Error("Deposit bank not found");
        const borrowBank = this.client.banks.get(borrowBankAddress.toBase58());
        ...
        // tiny priority fee just in case bundle fails
        const [priorityFeeIx] = (0, __1.makePriorityFeeIx)(0.00001);
        const borrowIxs = await this.makeBorrowIx(borrowAmount, borrowBankAddress, borrowOpts ?? {
            createAtas: false,
            wrapAndUnwrapSol: false,
        });
        const depositIxs = await this.makeDepositIx(depositAmount, depositBankAddress, depositOpts ?? {
            wrapAndUnwrapSol: false,
        });
        // unwrap if deposit bank is native
        const unwrapIx = depositBank.mint.equals(spl_token_1.NATIVE_MINT) ? [(0, __1.makeUnwrapSolIx)(this.authority)] : [];
        const { instructions: updateFeedIxs, luts: feedLuts } = await this.makeUpdateFeedIx([
            depositBankAddress,
            borrowBankAddress,
        ]);
    ...
    async makeDepositStakedTx(amount, bankAddress, stakeAccountPk, validator, depositOpts = {}) {
        ...
        if (!isFullStake) {
            ...
            }, rentExemptReserve).instructions);
        }
        else {
     ...
     */
    async simulateBorrowLendTransaction(txs, banksToInspect) {
    ...
    async transferAccountAuthority(newAccountAuthority, processOpts, txOpts) {
        ...
        const tx = new web3_js_1.Transaction().add(...ixs.instructions);
        const solanaTx = (0, mrgn_common_1.addTransactionMetadata)(tx, { type: mrgn_common_1.TransactionType.TRANSFER_AUTH });
        const sig = await this.client.processTransaction(solanaTx, processOpts, txOpts);
    ...
    // Helpers
    // --------------------------------------------------------------------------
    getHealthCheckAccounts(mandatoryBanks = [], excludedBanks = [], bankMetadataMap, authority) {
        return this._marginfiAccount.getHealthCheckAccounts(this.client.banks, mandatoryBanks, excludedBanks, bankMetadataMap);
    }
